...
class FedSGDClient(object):
    ...
    def train(self, round_idx = None):
        ...
        logging.info(f"data_id_num: " + str(len(self.train_local_list[0])))
        for data_id in range(len(self.train_local_list[0])):
            self.args.data_id = data_id
            logging.info(f"####### client training ########### round_id = {round_idx} data_id = {data_id}")
            self.trainer.train([train_local[data_id] for train_local in self.train_local_list], self.device, self.args)
        

        #weights = self.trainer.get_model_params()
        weights = [para.detach().cpu() for para in self.trainer.client_trainer.grad_zoo]
