...
class FedSGDServer(object):
    ...
    def aggregate(self,current_round):
        ...
        training_num = 0
        for idx in range(self.worker_num):
            local_sample_number, local_grads = self.sample_num_dict[idx], self.model_dict[idx]
            if averaged_grads is None:
                averaged_grads = [g.clone() for g in local_grads]
                ...
                for j, g in enumerate(local_grads):
                    ...
                    training_num += local_sample_number
        training_num *= 15
        logging.info("training number: " + str(training_num))
        for j in range(len(averaged_grads)):
            averaged_grads[j] /= training_num
        ...
        #######  老版本，next  ###
        # old_param = self.trainer.server_trainer.model.parameters()
        ...
        #         else:
        #             averaged_params[id] += local_model_params[id]
        #     next(old_param).data.to("cpu").sub_(learning_rate * averaged_params[id] / training_num)
